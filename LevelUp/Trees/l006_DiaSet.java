...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    public int maxPathSum(TreeNode root) {
     ...
     public int maxSinglePath(TreeNode node, int[] max) {
         if (node == null)   return 0;
         
         int left = maxSinglePath(node.left, max);
         int right = maxSinglePath(node.right, max);
         
         int maxSinglePathSum = node.val + Math.max(0, Math.max(left, right));
         max[0] = Math.max(max[0], Math.max(maxSinglePathSum, node.val + left + right));
...
public int maxPathSum(TreeNode root) {
    ...
    public int maxSinglePath(TreeNode node, int[] max) {
        ...
        int oneSidedSum = node.val + Math.max(0, Math.max(left, right));
        max[0] = getMax(max[0], oneSidedSum, node.val + left + right);
        return oneSidedSum;
    }
}
///////////////////////////////////////////////////////////////////////////////////////////////////////////==========
   ...
   public class NTNPair {
    ...
    public NTNPair maxPathSum_(TreeNode root){
        ...
        int oneSidedMax = root.val + Math.max(left.NTNMaxSum, right.NTNMaxSum);
        myAns.maxPossibleAns = getMax(oneSidedMax, root.val, left.maxPossibleAns, right.maxPossibleAns,
                                     left.NTNMaxSum + root.val + right.NTNMaxSum);
        
        myAns.NTNMaxSum = Math.max(root.val, oneSidedMax);
    ...
    // -1 : camera required, 0 : already covered, 1 : I have camera
